...
import * as legacyNamedImport from "buffer";

const modules = {
  "node:buffer": defaultImport,
  buffer: legacyImport,
  "* as buffer": legacyNamedImport,
};
for (const module in modules) {
  const { Buffer } = modules[module];
  ...
  describe(module, () => {
    ...
    describe("Buffer.isEncoding", () => {
      ...
      it("should return false when input is not a valid encoding name", () => {
        ...
        expect(Buffer.isEncoding(undefined as unknown as string)).toEqual(
          false
        );
