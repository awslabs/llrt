...
test(() => {
  ...
  assert_equals(
    decoder.decode(new Uint8Array([0xf0, 0x41, 0x42]), { stream: true }),
    "\uFFFDAB"
  );
  assert_equals(decoder.decode(), "");

  assert_equals(
    decoder.decode(new Uint8Array([0xf0, 0x41, 0xf0]), { stream: true }),
    "\uFFFDA"
  );
  assert_equals(decoder.decode(), "\uFFFD");
  ...
  assert_equals(
    decoder.decode(new Uint8Array([0x8f]), { stream: true }),
    "\uFFFD\uFFFD"
  );
  assert_equals(
    decoder.decode(new Uint8Array([0x92]), { stream: true }),
    "\uFFFD"
  );
  assert_equals(decoder.decode(), "");
