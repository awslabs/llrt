      ...
      promise_test(
        async () => {
          ...
          try {
            ...
            derived = await subtle.deriveKey(
        ...
        algorithmName +
          " deriveBits checks for all-zero value result with a key of order " +
          test.order
      );
    });
  }
  ...
  // Ensure the keys generated by each algorithm are valid for key derivation.
  {
    promise_test(
      async () => {
        let derived;
        try {
          let key = await subtle.generateKey({ name: algorithmName }, true, [
            "deriveKey",
            "deriveBits",
          ]);
          derived = await subtle.deriveKey(
