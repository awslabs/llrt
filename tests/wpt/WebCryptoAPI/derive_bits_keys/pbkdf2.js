function define_tests() {
  ...
  return setUpBaseKeys(passwords).then(function (allKeys) {
    ...
    Object.keys(derivations).forEach(function (passwordSize) {
      Object.keys(derivations[passwordSize]).forEach(function (saltSize) {
        Object.keys(derivations[passwordSize][saltSize]).forEach(
          function (hashName) {
            Object.keys(derivations[passwordSize][saltSize][hashName]).forEach(
              function (iterations) {
                var testName =
                  passwordSize +
                  " password, " +
                      ...
                      .then(
                        function (derivation) {
                          assert_true(
                            equalBuffers(
                              derivation,
                              derivations[passwordSize][saltSize][hashName][
                                iterations
                              ]
                            ),
                            "Derived correct key"
                ...
                derivedKeyTypes.forEach(function (derivedKeyType) {
                  ...
                  Object.keys(derivedKeyType.algorithm).forEach(
                    function (prop) {
                      testName +=
                            ...
                            return subtle.exportKey("raw", key).then(
                              function (buffer) {
                                assert_true(
                                  equalBuffers(
                                    buffer,
                                    derivations[passwordSize][saltSize][
                                      hashName
                                    ][iterations].slice(
                                      0,
                  ...
                  var badHash =
                    hashName.substring(0, 3) + hashName.substring(4);
                  subsetTest(
                    promise_test,
                    function (test) {
                      return subtle
              ...
              function (test) {
                return subtle
                  ...
                  .then(
                    function (derivation) {
                      assert_unreached(
                        "0 iterations should have thrown an error"
                      );
                    },
                    function (err) {
                      assert_equals(
                        err.name,
                        "OperationError",
                        "deriveBits with 0 iterations correctly threw OperationError: " +
                          err.message
                      );
                    }
                  );
                    ...
                    .then(
                      ...
                      function (err) {
                        assert_equals(
                          err.name,
                          "OperationError",
                          "derivekey with 0 iterations correctly threw OperationError: " +
                            err.message
                        );
                      }
                    );
  ...
  function setUpBaseKeys(passwords) {
    ...
    Object.keys(passwords).forEach(function (passwordSize) {
      var promise = subtle
        .importKey("raw", passwords[passwordSize], { name: "PBKDF2" }, false, [
          "deriveKey",
          "deriveBits",
        ])
        .then(
