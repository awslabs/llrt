function run_test(algorithmNames, slowTest) {
  ...
  function testSuccess(algorithm, extractable, usages, resultType, testTag) {
    ...
    promise_test(
      function (test) {
        return subtle
          ...
          .then(
            ...
            function (err) {
              assert_unreached(
                "generateKey threw an unexpected error: " + err.toString()
              );
            }
          )
          .then(
            async function (result) {
              if (resultType === "CryptoKeyPair") {
                await Promise.all([
                  ...
                  result.publicKey.algorithm.name.startsWith("RSA")
                    ? undefined
            ...
            function (err) {
              assert_unreached(
                "exportKey threw an unexpected error: " + err.toString()
              );
            }
          );
      },
      testTag +
        ": generateKey" +
        parameterString(algorithm, extractable, usages)
    );
  }
