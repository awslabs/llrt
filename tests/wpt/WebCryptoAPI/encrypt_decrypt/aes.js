function run_test() {
  ...
  passingVectors.forEach(function (vector) {
    var promise = importVectorKey(vector, ["encrypt", "decrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          return subtle
            .encrypt(vector.algorithm, vector.key, vector.plaintext)
            .then(
              function (result) {
                assert_true(
                  equalBuffers(result, vector.result),
                  "Should return expected result"
                );
              },
              function (err) {
                assert_unreached(
                  "encrypt error for test " + vector.name + ": " + err.message
                );
              }
            );
        }, vector.name);
      ...
      function (vector) {
        promise_test(function (test) {
          var operation = subtle
            .encrypt(vector.algorithm, vector.key, plaintext)
            .then(
              function (result) {
                assert_true(
                  equalBuffers(result, vector.result),
  ...
  passingVectors.forEach(function (vector) {
    var promise = importVectorKey(vector, ["encrypt", "decrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          return subtle
            .decrypt(vector.algorithm, vector.key, vector.result)
            .then(
              function (result) {
                assert_true(
              ...
              function (err) {
                assert_unreached(
                  "decrypt error for test " + vector.name + ": " + err.message
                );
              }
            );
  ...
  passingVectors.forEach(function (vector) {
    ...
    var promise = importVectorKey(vector, ["encrypt", "decrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          var operation = subtle
            .decrypt(vector.algorithm, vector.key, ciphertext)
            .then(
              function (result) {
                assert_true(
                  equalBuffers(result, vector.plaintext),
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importKey step for decryption: " +
            vector.name +
            " with altered ciphertext"
        );
      }
    );
  ...
  passingVectors.forEach(function (vector) {
    ...
    var promise = importVectorKey(badVector, ["decrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          return subtle
            .encrypt(vector.algorithm, vector.key, vector.plaintext)
            .then(
              function (result) {
                assert_unreached(
                  "should have thrown exception for test " + vector.name
                );
              },
              function (err) {
                assert_equals(
                  err.name,
                  "InvalidAccessError",
                  "Should throw an InvalidAccessError instead of " + err.message
                );
              }
            );
  ...
  passingVectors.forEach(function (vector) {
    ...
    var promise = importVectorKey(vector, ["encrypt", "decrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          return subtle.encrypt(algorithm, vector.key, vector.plaintext).then(
            function (result) {
              assert_unreached(
                "encrypt succeeded despite mismatch " +
                  vector.name +
                  ": " +
                  err.message
              );
            ...
            function (err) {
              assert_equals(
                err.name,
                "InvalidAccessError",
                "Mismatch should cause InvalidAccessError instead of " +
                  err.message
              );
            }
          );
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importKey step: " +
            vector.name +
            " with mismatched key and algorithm"
        );
      }
    );
  ...
  passingVectors.forEach(function (vector) {
    ...
    var promise = importVectorKey(badVector, ["encrypt"]).then(
      function (vector) {
        promise_test(function (test) {
          return subtle
            .decrypt(vector.algorithm, vector.key, vector.result)
            ...
            .then(
              ...
              function (err) {
                assert_equals(
                  err.name,
                  "OperationError",
                  "Should throw an OperationError instead of " + err.message
                );
              }
            );
        }, vector.name);
