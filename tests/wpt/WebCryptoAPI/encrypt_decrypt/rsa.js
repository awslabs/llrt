function run_test() {
  ...
  passingVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["encrypt"], ["decrypt"]).then(
      function (vectors) {
        ...
        promise_test(function (test) {
          return subtle
            .decrypt(vector.algorithm, vector.privateKey, vector.ciphertext)
            .then(
              function (plaintext) {
                assert_true(
                  equalBuffers(plaintext, vector.plaintext, "Decryption works")
                );
              },
              function (err) {
                assert_unreached(
                  "Decryption should not throw error " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
      ...
      function (vectors) {
        ...
        promise_test(function (test) {
          ...
          var operation = subtle
            .decrypt(vector.algorithm, vector.privateKey, ciphertext)
            .then(
              function (plaintext) {
                assert_true(
                  equalBuffers(plaintext, vector.plaintext, "Decryption works")
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " decryption with altered ciphertext"
        );
      }
    );
  ...
  passingVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["encrypt"], ["decrypt"]).then(
      function (vectors) {
        promise_test(function (test) {
          return subtle
            .decrypt(vector.algorithm, vector.publicKey, vector.ciphertext)
            .then(
              function (plaintext) {
                assert_unreached(
                  "Should have thrown error for using publicKey to decrypt in " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
              ...
              function (err) {
                assert_equals(
                  err.name,
                  "InvalidAccessError",
                  "Should throw InvalidAccessError instead of " + err.message
                );
              }
            );
  ...
  passingVectors.forEach(function (originalVector) {
    ...
    var promise = importVectorKeys(vector, ["encrypt"], ["unwrapKey"]).then(
      function (vectors) {
        // Get a one byte longer plaintext to encrypt
        promise_test(function (test) {
          return subtle
            ...
            .then(
              function (plaintext) {
                assert_unreached(
                  "Should have thrown error for no decrypt usage in " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
    ...
    var promise = importVectorKeys(vector, ["encrypt"], ["decrypt"]).then(
      function (vectors) {
        promise_test(function (test) {
          ...
          var operation = subtle
            .encrypt(vector.algorithm, vector.publicKey, plaintext)
            .then(function (ciphertext) {
              assert_equals(
                ciphertext.byteLength * 8,
                vector.privateKey.algorithm.modulusLength,
                "Ciphertext length matches modulus length"
              );
              ...
              return subtle
                ...
                .then(
                  function (result) {
                    assert_true(
                      equalBuffers(result, vector.plaintext),
                      "Round trip returns original plaintext"
                    );
                    return ciphertext;
                  },
                  function (err) {
                    assert_unreached(
                      "decrypt error for test " +
                        vector.name +
                        ": " +
                        err.message +
                        "'"
            ...
            .then(
              function (priorCiphertext) {
                // Will a second encrypt give us different ciphertext, as it should?
                return subtle
                  .encrypt(vector.algorithm, vector.publicKey, vector.plaintext)
                  .then(
                    function (ciphertext) {
                      assert_false(
                        equalBuffers(priorCiphertext, ciphertext),
                        "Two encrypts give different results"
                      );
                    },
                    function (err) {
                      assert_unreached(
                        "second time encrypt error for test " +
                          vector.name +
                          ": '" +
                          err.message +
                          "'"
        ...
        promise_test(function (test) {
          return subtle
            ...
            .then(
              function (ciphertext) {
                assert_unreached(
                  "Should have thrown error for too long plaintext in " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
              ...
              function (err) {
                assert_equals(
                  err.name,
                  "OperationError",
                  "Should throw OperationError instead of " + err.message
                );
              }
            );
  ...
  passingVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["encrypt"], ["decrypt"]).then(
      function (vectors) {
        promise_test(function (test) {
          return subtle
            .encrypt(vector.algorithm, vector.privateKey, vector.plaintext)
            .then(
              function (ciphertext) {
                assert_unreached(
                  "Should have thrown error for using privateKey to encrypt in " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
  ...
  passingVectors.forEach(function (originalVector) {
    ...
    var promise = importVectorKeys(vector, [], ["decrypt"]).then(
      function (vectors) {
        // Get a one byte longer plaintext to encrypt
        promise_test(function (test) {
          return subtle
            ...
            .then(
              function (ciphertext) {
                assert_unreached(
                  "Should have thrown error for no encrypt usage in " +
                    vector.name +
                    ": " +
                    err.message +
                    "'"
    ...
    if (vector.publicKey !== null) {
      ...
      publicPromise = subtle
        .importKey(
          ...
          { name: vector.algorithm.name, hash: vector.hash },
          false,
          publicKeyUsages
        )
        .then(function (key) {
