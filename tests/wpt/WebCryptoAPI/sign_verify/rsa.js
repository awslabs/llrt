function run_test() {
  ...
  testVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        promise_test(function (test) {
          return subtle
            .sign(vector.algorithm, vector.publicKey, vector.plaintext)
            .then(
              function (signature) {
                assert_unreached(
  ...
  testVectors.forEach(function (vector) {
    ...
    const isDeterministic =
      !("saltLength" in vector.algorithm) || vector.algorithm.saltLength == 0;
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        promise_test(function (test) {
          return subtle
            .sign(vector.algorithm, vector.privateKey, vector.plaintext)
                ...
                return subtle
                  ...
                  .then(
                    function (signature) {
                      if (isDeterministic) {
                        assert_true(
                          equalBuffers(priorSignature, signature),
                    ...
                    function (err) {
                      assert_unreached(
                        "second time verify error for test " +
                          vector.name +
                          ": '" +
                          err.message +
                          "'"
  ...
  testVectors.forEach(function (vector) {
    ...
    var promise = importVectorKeys(alteredVector, ["verify"], ["sign"]).then(
      function (vectors) {
        promise_test(function (test) {
          var operation = subtle
            .sign(vector.algorithm, alteredVector.privateKey, vector.plaintext)
            .then(
              function (signature) {
                assert_unreached(
    ...
    var promise = importVectorKeys(alteredVector, ["verify"], ["sign"]).then(
        ...
        promise_test(function (test) {
          var operation = subtle
            ...
            .then(
              function (is_verified) {
                assert_unreached(
                  "Verification should not have succeeded for " + vector.name
                );
              },
              function (err) {
                assert_equals(
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " verification with wrong algorithm name"
        );
      }
    );
    ...
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " verification failure with altered signature"
        );
      }
    );
      ...
      var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
        ...
        function (err) {
          ...
          promise_test(
            ...
            "importVectorKeys step: " +
              vector.name +
              " verification failure with wrong saltLength"
          );
        }
      );
    ...
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " verification failure with altered plaintext"
        );
      }
    );
