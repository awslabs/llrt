    ...
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        ...
        promise_test(function (test) {
          ...
          var operation = subtle
            .verify(algorithm, vector.publicKey, signature, vector.plaintext)
            .then(
              function (is_verified) {
                assert_true(is_verified, "Signature verified");
              },
  ...
  testVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        ...
        promise_test(function (test) {
          return subtle
            .sign(algorithm, vector.publicKey, vector.plaintext)
            .then(
              function (signature) {
                assert_unreached(
  ...
  testVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        ...
        promise_test(function (test) {
          return subtle
            .sign(algorithm, vector.privateKey, vector.plaintext)
            .then(
              function (signature) {
        ...
        function (wrongKey) {
          ...
          return importVectorKeys(vector, ["verify"], ["sign"]).then(
            function (vectors) {
              promise_test(function (test) {
                var operation = subtle
                  .sign(algorithm, wrongKey, vector.plaintext)
                  .then(
                    function (signature) {
                      assert_unreached(
    ...
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " verification failure due to wrong hash"
        );
      }
    );
  ...
  testVectors.forEach(function (vector) {
    var promise = importVectorKeys(vector, ["verify"], ["sign"]).then(
      function (vectors) {
        ...
        var hashName =
          vector.hashName.substring(0, 3) + vector.hashName.substring(4);
        var algorithm = { name: vector.algorithmName, hash: hashName };
        promise_test(function (test) {
          var operation = subtle
            .verify(
      ...
      function (err) {
        ...
        promise_test(
          ...
          "importVectorKeys step: " +
            vector.name +
            " verification failure due to bad hash name"
        );
      }
    );
    ...
    if (vector.publicKey !== null) {
      ...
      publicPromise = subtle
        .importKey(
          ...
          { name: vector.algorithmName, namedCurve: vector.namedCurve },
          false,
          publicKeyUsages
        )
        .then(function (key) {
