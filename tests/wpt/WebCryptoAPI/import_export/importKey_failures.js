function run_test(algorithmNames) {
  ...
  var allTestVectors = [
    // Parameters that should work for importKey / exportKey
    ...
    promise_test(
      async () => {
        ...
        try {
          ...
          let actualError =
            typeof expectedError === "number" ? err.code : err.name;
          assert_equals(
            actualError,
            expectedError,
            testTag + " not supported."
          );
        }
        assert_equals(
          key,
      ...
      testTag +
        ": importKey" +
        parameterString(format, algorithm, extractable, usages, keyData)
    );
  }
  ...
  function invalidUsages(validUsages, mandatoryUsages) {
    ...
    var goodUsageCombinations =
      validUsages.length === 0
        ? []
        : allValidUsages(validUsages, false, mandatoryUsages);
  ...
  testVectors.forEach(function (vector) {
    ...
    allAlgorithmSpecifiersFor(name).forEach(function (algorithm) {
      getValidKeyData(algorithm).forEach(function (test) {
        invalidUsages(validUsages(vector, test.format, test.data)).forEach(
          function (usages) {
  ...
  testVectors.forEach(function (vector) {
    var name = vector.name;
    allAlgorithmSpecifiersFor(name).forEach(function (algorithm) {
      getValidKeyData(algorithm)
        .filter(
          (test) =>
            test.format === "pkcs8" ||
            (test.format === "jwk" && isPrivateKey(test.data))
        )
  ...
  testVectors.forEach(function (vector) {
    ...
    allAlgorithmSpecifiersFor(name).forEach(function (algorithm) {
      getBadKeyLengthData(algorithm).forEach(function (test) {
        allValidUsages(validUsages(vector, test.format, test.data)).forEach(
          function (usages) {
            [true, false].forEach(function (extractable) {
              testError(
                test.format,
  ...
  testVectors.forEach(function (vector) {
    ...
    allAlgorithmSpecifiersFor(name).forEach(function (algorithm) {
      getMissingJWKFieldKeyData(algorithm).forEach(function (test) {
        allValidUsages(validUsages(vector, "jwk", test.data)).forEach(
          function (usages) {
            [true, false].forEach(function (extractable) {
              testError(
                "jwk",
                ...
                extractable,
                "DataError",
                "Missing JWK '" + test.param + "' parameter"
              );
            });
          }
        );
  ...
  testVectors.forEach(function (vector) {
    var name = vector.name;
    if (name !== "Ed25519" && name !== "Ed448") return; // The rest ignore the 'alg' field.
    allAlgorithmSpecifiersFor(name).forEach(function (algorithm) {
      getValidKeyData(algorithm).forEach(function (test) {
        if (test.format === "jwk") {
          var data = {
