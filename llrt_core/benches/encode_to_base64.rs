...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("[JS] Buffer::from(seed).toString('base64')", |b| {
        ctx.with(|ctx| {
            b.iter(|| ctx.eval::<String, &[u8]>(b"Buffer.from(seed).toString('base64')"))
        });
    });

    c.bench_function("[JS] Uint8Array.toBase64(seed)", |b| {
        ctx.with(|ctx| b.iter(|| ctx.eval::<String, &[u8]>(b"seed.toBase64()")));
    });

    c.bench_function("[JS] llrt:codec.encodeToBase64(seed)", |b| {
        ctx.with(|ctx| b.iter(|| ctx.eval::<String, &[u8]>(b"encodeToBase64(seed)")));
    });

    c.bench_function("[Native] llrt_encoding::bytes_to_b64(seed)", |b| {
